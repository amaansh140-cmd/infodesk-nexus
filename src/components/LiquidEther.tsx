...
const LiquidEther = memo(({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      isAutoActive = true; // Always active for ambient flow
      autoIntensity = 2.0;
      ...
      init(container: HTMLElement) {
        ...
        this.listenerTarget.addEventListener('mousemove', this._onMouseMove as EventListener);
      ...
      dispose() {
        if (this.listenerTarget) {
          this.listenerTarget.removeEventListener('mousemove', this._onMouseMove as EventListener);
      ...
      setCoords(x: number, y: number) {
        ...
        this.timer = window.setTimeout(() => { this.mouseMoved = false; }, 100) as any;
      }
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          ...
          this.takeoverActive = true; this.hasUserControl = true;
          return;
        }
    ...
    const AutoMouse = new AutoMouseClass();
    ...
    class AutoDriver {
      ...
      update() {
        ...
        AutoMouse.coords.copy(this.current);
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      autoMouse: THREE.Mesh | null = null;
      ...
      initForce(simProps: any) {
        ...
        this.mouse = new THREE.Mesh(new THREE.PlaneGeometry(1,1), mat());
        this.autoMouse = new THREE.Mesh(new THREE.PlaneGeometry(1,1), mat());
      ...
      update(props: any) {
        ...
        u.force.value.set(fX, fY); u.center.value.set(cX, cY); u.scale.value.set(props.cursor_size, props.cursor_size);
        
        // Update Auto Force (Ambient)
        const afX = (AutoMouse.diff.x / 2) * props.mouse_force * 0.8; // Slightly weaker
        const afY = (AutoMouse.diff.y / 2) * props.mouse_force * 0.8;
        const acX = Math.min(Math.max(AutoMouse.coords.x, -1+cSX+props.cellScale.x*2), 1-cSX-props.cellScale.x*2);
        const acY = Math.min(Math.max(AutoMouse.coords.y, -1+cSY+props.cellScale.y*2), 1-cSY-props.cellScale.y*2);
        const au = (this.autoMouse!.material as THREE.RawShaderMaterial).uniforms;
        au.force.value.set(afX, afY); au.center.value.set(acX, acY); au.scale.value.set(props.cursor_size * 1.5, props.cursor_size * 1.5); // Larger swirls
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous, iterations, dt }: any) {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          this.props.output = fbo_out; this.uniforms.dt.value = dt; super.update();
        }
        return fbo_out;
      }
    }
    ...
    class Simulation {
      ...
      advection: Advection | null = null; externalForce: ExternalForce | null = null; viscous: Viscous | null = null;
      divergence: Divergence | null = null; poisson: Poisson | null = null; pressure: Pressure | null = null;
    ...
    class WebGLManager {
      props: any; output: OutputClass | null = null; autoDriver: AutoDriver | null = null;
      ...
      constructor(props: any) {
        ...
        Mouse.onInteract = () => { this.lastUserInteraction = performance.now(); };
      ...
      render() { if (this.autoDriver) this.autoDriver.update(); AutoMouse.update(); Mouse.update(); Common.update(); this.output!.update(); }
    ...
    return () => {
      ...
      try { resizeObserverRef.current?.disconnect(); } catch (_) { void 0; }
      try { intersectionObserverRef.current?.disconnect(); } catch (_) { void 0; }
