...
export default function StudentTimetable() {
  const schedule = [
    { day: 'Monday', slots: [
      { time: '09:00 AM - 10:30 AM', course: 'Data Structures', faculty: 'Prof. Smith', room: 'Room 201', type: 'Lecture' },
      { time: '11:00 AM - 12:30 PM', course: 'Algorithms', faculty: 'Prof. Smith', room: 'Room 204', type: 'Lecture' },
    ]},
    { day: 'Tuesday', slots: [
      { time: '09:00 AM - 12:00 PM', course: 'Web Development', faculty: 'Prof. Davis', room: 'Lab 1', type: 'Practical' },
    ]},
    { day: 'Wednesday', slots: [
      { time: '10:00 AM - 11:30 AM', course: 'Database Management', faculty: 'Prof. Johnson', room: 'Room 305', type: 'Lecture' },
    ]},
    { day: 'Thursday', slots: [] },
    { day: 'Friday', slots: [
      { time: '10:00 AM - 11:30 AM', course: 'Data Structures', faculty: 'Prof. Smith', room: 'Room 201', type: 'Tutorial' },
    ]},
  ];
    ...
    <div className={styles.pageContainer}>
      <header className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>Your weekly class schedule for Semester 3 (Batch A)</p>
        </div>
        <button className={styles.secondaryBtn}>
          <CalendarIcon size={16} /> Sync to Personal Calendar
        </button>
      </header>
                    ...
                    {dayData.slots.map((slot, sIdx) => (
                      <div key={sIdx} style={{ padding: '1rem', borderRadius: '0.5rem', background: 'white', border: '1px solid #e5e7eb', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                          ...
                          <span className={styles.badge} style={{ color: slot.type === 'Practical' ? '#8b5cf6' : '#3b82f6' }}>{slot.type}</span>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.875rem', color: '#6b7280' }}>
                          ...
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><MapPin size={14} /> {slot.room}</span>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><User size={14} /> {slot.faculty}</span>
                        </div>
                      </div>
                    ))}
                  </div>
